...
<body>
		...
		<div class="container-fluid" id="pcont">
			<div class="page-head">
				<ol class="breadcrumb">
					<li>首页</li>
					<li class="active">
						<c:if test="${type=='1' }">学校信息</c:if>
						<c:if test="${type=='2' }">公寓信息</c:if>
					</li>
				</ol>
			</div>
			<div class="cl-mcont">
				<c:if test="${roler=='0' }">
					<div class="row">
						<div class="col-md-12">
							<a href="<%=basePath%>news/beforeAddOrUpdateNews?newsId=&type=${type}" class="btn btn-primary btn-flat">
								<c:if test="${type=='1' }">添加学校信息</c:if>
								<c:if test="${type=='2' }">添加公寓信息</c:if>
							</a>
						</div>
					</div>
				</c:if>
							...
							<div class="content">
								<div class="table-responsive">
									<table class="table table-bordered" id="datatable">
										...
										<tbody>
											<c:forEach items="${newsList }" var="obj" varStatus="_v">
												<tr class="odd gradeX">
													<td>${_v.count }</td>
													<td>${obj.title }</td>
													<td>${obj.content }</td>
													...
													<c:if test="${roler=='0' }">
														<td>
															<a class="btn btn-warning btn-xs" href="<%=basePath%>news/beforeAddOrUpdateNews?newsId=${obj.newsid }&type=${type}"><i class="fa fa-pencil"></i></a>
															<button class="btn btn-danger btn-xs btn-flat md-trigger" data-modal="colored-warning" onclick="javascript:$('#delId').val('${obj.newsid}');"><i class="fa fa-times"></i></button>
														</td>
													</c:if>
												</tr>
											</c:forEach>
	...
	<div class="md-modal colored-header warning md-effect-10" id="colored-warning">
		<div class="md-content">
			<form action="<%=basePath%>news/deleteNews" method="post">
	            ...
	            <div class="modal-body">
					<div class="text-center">
						<h4>删除后无法恢复，请谨慎操作！！！</h4>
						<input type="hidden" name="newsId" id="delId">
