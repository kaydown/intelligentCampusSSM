		...
		<div class="container-fluid" id="pcont">
			...
			<div class="cl-mcont">
				<c:if test="${roler=='0' }">
					<div class="row">
						<div class="col-md-12">
							<a href="<%=basePath%>user/beforeAddOrUpdateUser?userId=" class="btn btn-primary btn-flat">添加学生</a>
						</div>
					</div>
				</c:if>
				<div class="row">
										...
										<tbody>
											<c:forEach items="${userList }" var="obj" varStatus="_v">
												<tr class="odd gradeX">
													...
													<td>${obj.content }</td>
													<td>
														<a class="btn btn-warning btn-xs" href="<%=basePath%>user/beforeAddOrUpdateUser?userId=${obj.userid}"><i class="fa fa-pencil"></i></a>
														<button class="btn btn-danger btn-xs btn-flat md-trigger" data-modal="colored-warning" onclick="javascript:$('#delId').val('${obj.userid}');"><i class="fa fa-times"></i></button>
													</td>
												</tr>
											</c:forEach>
										</tbody>
			...
			<form action="<%=basePath%>user/deleteUser" method="post">
	            ...
	            <div class="modal-body">
					<div class="text-center">
						<h4>删除后无法恢复，请谨慎操作！！！</h4>
						<input type="hidden" name="userId" id="delId">
					</div>
				</div>
